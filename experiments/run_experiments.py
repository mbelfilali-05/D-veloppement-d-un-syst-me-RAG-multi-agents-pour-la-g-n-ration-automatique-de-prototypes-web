...
def run_single_experiment(
    ...
    try:
        ...
        # 2. Instancie CRAgent et reconstruit sa chain avec le bon prompt.
        #    On passe les paramètres optionnels seulement s'ils existent
        #    dans le constructeur, pour rester compatible avec toutes les versions.
        try:
            agent = CRAgent(
            ...
            # Ancienne version de CRAgent sans ces paramètres :
            # on instancie normalement puis on recompile la chain manuellement
            from langchain_core.prompts import ChatPromptTemplate
            from langchain_core.output_parsers import StrOutputParser
            agent = CRAgent(vector_store=vector_store)
            prompt = ChatPromptTemplate.from_template(config.prompt_template)
            agent.chain = prompt | agent.llm | StrOutputParser()
